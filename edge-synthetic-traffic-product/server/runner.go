...
func runBERTTest(target string, ber *float64) error {
	...
	// Bolt optimization: Precalculate 256-byte repeating base pattern
	// and use logarithmic doubling with copy() for faster slice initialization
	// This reduces CPU time spent generating the payload before sending
	for i := 0; i < 256; i++ {
