...
## 2026-05-01 - Destructive Actions Without Confirmation

**Learning:** Destructive actions, such as deleting a task, that are initiated with a single click without any confirmation dialog can easily lead to accidental data loss. This is especially problematic if the action button is revealed on hover.

**Action:** Always wrap destructive UI actions (like deleting items) in a confirmation mechanism (e.g., `window.confirm`) to provide users with a safety net and adhere to expected UX standards.
