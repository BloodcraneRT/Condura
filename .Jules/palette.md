...
## 2024-04-19 - Confirmation Dialog for Destructive Actions\n**Learning:** Destructive actions like deleting a task should not execute immediately to prevent accidental data loss.\n**Action:** Always wrap destructive API calls (like DELETE requests) triggered by UI buttons in a `window.confirm` dialog or similar confirmation mechanism to improve safety and adhere to project UX standards.
