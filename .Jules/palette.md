...
**Learning:** Destructive actions without a confirmation dialog can cause accidental data loss and violate UX best practices for safety and error prevention.
**Action:** Always wrap destructive UI actions (such as deletion) with a confirmation dialog (e.g., `window.confirm`) to ensure the user intends to perform the action.
