...
## 2026-04-08 - Add explicit visual indicators for required fields and loading states
**Learning:** Users lack confidence when async operations lack immediate visual feedback (like button states or text changes), and ambiguous forms without explicit required indicators (asterisks) lead to validation frustration.
**Action:** Always add `aria-hidden` red asterisks to required form labels, and explicitly handle loading states by disabling submit buttons and updating their text (e.g., 'Saving...') to prevent duplicate submissions and provide immediate feedback.
