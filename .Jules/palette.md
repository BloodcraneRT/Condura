...
**Learning:** Destructive actions without confirmation dialogs (like deleting tasks immediately on click) are a significant UX risk, leading to accidental data loss and violating standard defensive design principles.
**Action:** Always wrap state-mutating, destructive UI actions (e.g., DELETE API calls) in a `window.confirm()` dialog with clear, descriptive text (e.g., "Are you sure you want to delete this [type] task?").
