...
## 2024-05-21 - Destructive Actions and Accidental Data Loss
**Learning:** Destructive actions without user confirmation lead to accidental data loss. This is especially true for items in lists or tables where a stray click could easily delete an item. A missing confirmation step creates anxiety and frustration for users.
**Action:** Always intercept destructive actions, such as "Delete," with a confirmation step. For a lightweight, native approach, use `window.confirm("Are you sure?")` to prompt the user before calling the API to actually remove data.
