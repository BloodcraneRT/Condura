...
## 2024-05-20 - Accessible Required Indicators in Forms
**Learning:** Adding complex nested elements (like required indicator asterisks) inside form `<label>` tags can sometimes cause screen readers or testing libraries (like Playwright's `get_by_label`) to incorrectly parse the accessible name of the label, as they read the raw text contents. For example, a label `Name *` is sometimes parsed differently than expected.
**Action:** Always wrap visual required indicators (e.g., `*`) in a `<span>` with `aria-hidden="true"` when placing them inside `<label>` elements. This ensures they are visible to sighted users but completely ignored by assistive technologies, keeping the accessible name clean and predictable (e.g., "Name"). Furthermore, when writing tests, explicit ID locators are more reliable when labels contain complex hidden sub-elements.
