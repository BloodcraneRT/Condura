...
## 2026-05-05 - Confirm Dialog Accessibility for Destructive Actions
**Learning:** Destructive actions hidden behind hover states (like the delete task button) are easy to misclick, and a missing confirmation step means immediate data loss. Using `window.confirm` provides a native, accessible way to pause the execution and ask the user for confirmation without introducing complex React state or custom dialog components that might fail screen reader checks.
**Action:** Always wrap `fetch(..., {method: 'DELETE'})` calls inside a simple `if (!window.confirm("...")) return;` guard to ensure the user intends the destructive action.
