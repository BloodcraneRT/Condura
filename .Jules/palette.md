...
## 2024-04-11 - Form Loading States & Accessible Required Indicators
**Learning:** In this project's component architecture, standard HTML5 validation and required fields must be explicitly paired with visual indicators (like red asterisks). Using `<span aria-hidden="true" className="text-rose-500 ml-1">*</span>` within explicitly linked `<label htmlFor="...">` elements provides clear visual feedback without disrupting screen reader flow. Additionally, immediate visual feedback (like disabling submit buttons and updating their text to "Saving...") is critical for long-running async operations to prevent double submissions.
**Action:** Always include explicit `id` and `htmlFor` linkages, explicitly handle loading states by disabling submit buttons and updating their text, and use accessible visual indicators for required fields in all new forms.
