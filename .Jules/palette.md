...
## 2026-04-29 - Destructive Actions Without Confirmation

**Learning:** Destructive actions, such as deleting a task, that execute immediately upon clicking a button create a high risk for accidental data loss, especially when the actions are close to other interactive elements.

**Action:** Always wrap destructive UI actions (e.g., deleting tasks, configurations) in a native confirmation dialog (like `if (!window.confirm("...")) return;`) to provide a safety net for users and adhere to UX standards, while keeping the code implementation minimal.
